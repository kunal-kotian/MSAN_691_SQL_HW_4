...
first_database <- dbConnect(drv, dbname = "msan691", host = "localhost", port = 5432, user = "hollycapell", password = "postgres")
...
q_6 <- c("select num_successes::float / (select (count(distinct permno) * 11) from stocks2016.d2010) as probability
from
...
```{r R.options = list(max.print = 10)}
q_LTV1 <- c("select sum(case when dt = min_date and transtype = 'Unit' and subs > 0 then 1 else 0 end)::float/count(distinct userid) as percent
from
(select userid, transtype, dt, min(dt) over (partition by userid) as min_date,
sum(case when transtype = 'Subscription' then 1 else 0 end) over (partition by userid) as subs from 
cls.ltv) q1;")
...
from
(select cohort, row_number() over (partition by cohort, month, rev/lag(rev) over (partition by cohort order by month)) as total_rev 
from 
