...
(b)
```{r R.options = list(max.print = 100)}
qn_4a2b <- c("select avg(vol), date_part('dow',retdate::date) as daysofweek from stocks2016.d2010
...
(c)
```{r R.options = list(max.print = 100)}
qn_4a2c <- c("select avg(cast(ret as numeric)),date_part('month', retdate::date) as months, date_part('dow', retdate::date) as daysofweek from stocks2016.d2010
...
from
    ...
    from stocks2016.d2010
    where prc is not null and date_part('dow', retdate) = 2 and date_part('month', retdate) = 1
    group by permno
...
from
    (select permno, max(abs(prc)) as max_price, min(abs(prc)) as min_price
    from stocks2016.d2010
    where prc is not null and date_part('dow', retdate) = 3 and date_part('month', retdate) = 1
    group by permno
    having count(distinct retdate) = 4) as InnerQ;")
...
```{r R.options = list(max.print = 10)}
qn_4b2a <- c("select avg(ct) as avgct
from
   (select avg(swpeakhr) as ct, county
    from    cls.traffic
    where county like 'A%'
    group by county) as innerQ;")
...
```{r R.options = list(max.print = 10)}
qn_4b2c <- c("select avg(ct) as avgct
from
   (select avg(swpeakhr) as ct, county
    from    cls.traffic
    where county like 'A%' or county like 'M%'
    group by county) as innerQ;")
...
```{r R.options = list(max.print = 10)}
qn_4b2d <- c("select 
    avg(case when county like 'A%' then avg_speed end) as A_type,
    avg(case when county like 'M%' then avg_speed end) as B_type
from(
    select county, avg(swpeakhr) as avg_speed
    from cls.traffic 
    group by county 
    )as q2;")
...
```{r R.options = list(max.print = 10)}
qn_4b2e <- c("select county_type, avg(avg_1) as avg_total
from(
    select 
    case when county like 'A%'then 'type_A'
         when county like 'M%'then 'type_M'
    end as county_type,avg_1
    from(select county, avg(swpeakhr) as avg_1
         from cls.traffic
         group by county 
    ) as q3)as q4
where county_type is not null
group by county_type;")
dbGetQuery(first_database,qn_4b2e)
```
...
```{r R.options = list(max.print = 10)}
qn_4b2f <- c("select
    count(distinct routeno) as no_route
from cls.traffic
...
```{r R.options = list(max.print = 10)}
qn_4b2g <- c("select avg(ratio) as ratio_total
from(
select county, routeno,avg(nepeakhr/swpeakhr) as ratio
from cls.traffic
...
```{r R.options = list(max.print = 10)}
qn_4b2j <- c("select 
    count(case when swmaxday > 15000 then 1 else null end) as big_part,
    count(case when swmaxday between 10000 and 15000 then 1 else null end) as medium_part,
    count(case when swmaxday between 0 and 10000 then 1 else null end) as small_part
from( select county, routeno, max(swavgday) as swmaxday 
      from cls.traffic group by county, routeno
    )q1
where county IN(select county from cls.traffic where swavgday > 15000);
...
```{r R.options = list(max.print = 10)}
qn_4c2a <- c("SELECT round(cast(ret as numeric),5)*100 as return, (abs(vol*prc::bigint)/1000)*1000 as dollar_volume  FROM stocks2016.d2010
...
#With R approximately 0 there is no apperant linear relationship between return and dollar volume

(d)
```{r R.options = list(max.print = 10)}
qn_4c2d <- c("SELECT round(cast(ret as numeric),5)*100 as return, (abs(vol*prc::bigint)/1000)*1000 as dollar_volume  FROM stocks2016.d2010
where  (ret::numeric) between -.1 and .1 and (abs(vol*prc::bigint)/1000)*1000 > 250000000 and ret != 'C' and ret != 'B' and md5(permno::varchar(100)) like '0%';")
...
#there does to seem to be alinear relationship between return and dollar volume 
...
(f)
```{r R.options = list(max.print = 10)}
qn_4c2f <- c("select avg(return^2)-avg(return)^2 as variance_return, avg(dollar_volume^2)-avg(dollar_volume)^2 as dollar_value_variance from (SELECT round(cast(ret as numeric),5)*100 as return, (abs(vol*prc::bigint)/1000)*1000 as dollar_volume  FROM stocks2016.d2010
where  ret != 'C' and ret != 'B' and md5(permno::varchar(100)) like '0%') as innerq;")
...
(g)
```{r R.options = list(max.print = 10)}
qn_4c2g <- c("select avg(return * dollar_volume) - (avg(return)*avg(dollar_volume)) as covariance_of_return_dollar_value from 
(SELECT round(cast(ret as numeric),5)*100 as return, (abs(vol*prc::bigint)/1000)*1000 as dollar_volume  FROM stocks2016.d2010
where  ret != 'C' and ret != 'B' and md5(permno::varchar(100)) like '0%') as innerq;")
